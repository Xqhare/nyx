...
use talos::{
    ...
    layout::Rect,
    render::{Canvas, Style},
    widgets::{Area, Block, Text, stateful::Table, traits::Widget},
};
...
fn right_bottom(
    ...
    let area = layout
        .get("body_bottom_right_bottom")
        .expect("area must exist");
    ...
    if *dockerstate == XffValue::Null {
        ...
        let mut dock_text = Text::new("Docker not running", codex)
            .align_center()
            .align_vertically();
        ...
        dock_area.style(style);
        dock_area.render(canvas, area, codex);
        let mut dock_text = Text::new(dockerstate.to_string(), codex)
            .align_center()
            .align_vertically();
        dock_text.style(style);
        dock_text.render(canvas, area, codex);
    ...
    rows.push(vec![
        Text::new("ID", codex),
        Text::new("Name", codex),
        Text::new("State", codex),
        Text::new("Image", codex),
    ]);

    let docker_state = dockerstate
        .as_object()
        .expect("Docker state must be an object");
...
fn main_right(
    ...
    let df_state = state
        .get("df")
        .expect("Df state not found")
        .as_object()
        .expect("Df state must be an object");
    let area = layout
        .get("body_bottom_right_top")
        .expect("Top area must exist");
    ...
    let mut rows: Vec<Vec<Text>> = Vec::new();
    rows.push(vec![
        Text::new("Filesystem", codex),
        Text::new("Size", codex),
        Text::new("Used", codex),
        Text::new("Avail", codex),
        Text::new("Use%", codex),
        Text::new("Mounted on", codex),
    ]);
    ...
    for (_, value) in df_state.iter() {
        ...
        let mounted_on = value
            .get("Mounted on")
            .expect("Mounted on must exist")
            .to_string();
        rows.push(vec![
            Text::new(filesystem, codex),
            Text::new(size, codex),
