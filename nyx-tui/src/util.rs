use talos::{LayoutBuilder, layout::{Constraint, Direction, Rect}, render::{Bright, Colour, Extended, Normal, Style, TrueColour}};
...
pub fn layouter(rect: Rect) -> BTreeMap<String, Rect> {
    ...
    let (body_top, body_bottom) = {
        let body_layout = LayoutBuilder::new().direction(Direction::Horizontal).add_constraint(Constraint::Percentage(10)).add_constraint(Constraint::Percentage(90)).margin(1).build().split(body);
...
pub fn make_style_atlas() -> BTreeMap<String, Style> {
    BTreeMap::from([
        ...
        ("head".to_string(), Style::builder()
            .set_fg(
                Colour::Extended(
                    Extended::TrueColour(
                        TrueColour::RGB(200, 200, 200)
                    )
                )
            )
            .set_bg(Colour::Normal(Normal::Black))
        ...
        ("highlight".to_string(), Style::builder()
